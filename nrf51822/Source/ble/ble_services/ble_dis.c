...
static ble_gatts_char_handles_t my_name_handles;
...
uint32_t ble_dis_init(const ble_dis_init_t * p_dis_init)
{
    ...
    if (p_dis_init->manufact_name_str.length > 0)
    {
		...
		if (p_dis_init->my_name_str.length > 0)
    {
        err_code = char_add(0x3000,
                            p_dis_init->my_name_str.p_str, 
                            p_dis_init->my_name_str.length,
                            &p_dis_init->dis_attr_md,
                            &my_name_handles);
        if (err_code != NRF_SUCCESS)
        {
