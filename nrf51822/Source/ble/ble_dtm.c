...
/**@brief Verify that a received PDU has the expected structure and content.
 */
...
/**@brief Turn off radio after test.
...
/**@brief Initializing the radio for DTM.
 */
...
/**@brief At start of each test: Turn off RF, clear interrupt flags of RF, inititialize the radio
 *        at given RF channel.
 *
...
/**@brief Terminate ongoing test (if any) and close down radio.
 */
static void dtm_test_done(void)
{
    NRF_RADIO->TEST = 0;
    NRF_PPI->CHENCLR = 0x01;
    NRF_PPI->CH0_EEP = 0;     // Break connection from timer to radio to stop transmit loop
    NRF_PPI->CH0_TEP = 0;
...
/**@brief Configure timer for 625us cycle time.
 */
...
/**@brief Function to handle vendor specific commands.
...
uint32_t dtm_cmd(dtm_cmd_t cmd, dtm_freq_t freq, uint32_t length, dtm_pkt_type_t payload)
{
    ...
    if (cmd == LE_TRANSMITTER_TEST)
    {
        ...
        NRF_PPI->CH0_EEP = (uint32_t)&mp_timer->EVENTS_COMPARE[0];
        NRF_PPI->CH0_TEP = (uint32_t)&NRF_RADIO->TASKS_TXEN;
...
/**@brief Configure output power for transmitter test.
