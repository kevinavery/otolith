...
static void services_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
		...
		// Initialize OUR Accelerometer Service
    memset(&acc_init, 0, sizeof(acc_init));

    ble_srv_ascii_to_utf8(&acc_init.manufact_name_str, MANUFACTURER_NAME);

    BLE_GAP_CONN_SEC_MODE_SET_OPEN(&acc_init.acc_attr_md.read_perm);
    BLE_GAP_CONN_SEC_MODE_SET_NO_ACCESS(&acc_init.acc_attr_md.write_perm);

    err_code = ble_acc_init(&acc_init);
    APP_ERROR_CHECK(err_code);
}
